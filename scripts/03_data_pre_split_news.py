...
apple_prices["date"] = apple_prices["timestamp"].dt.date

# Calculate daily returns
daily = apple_prices.groupby("date").agg({"open":"first", "close":"last"})
daily["daily_return"] = (daily["close"] - daily["open"]) / daily["open"]

# Merge daily returns back to the news DataFrame
df = df.merge(daily[["daily_return"]], on="date", how="left")
