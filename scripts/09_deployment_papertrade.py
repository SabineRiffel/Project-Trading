...
df = pd.read_csv("../data/paper_trading_log.csv")
df["timestamp"] = pd.to_datetime(df["timestamp"])

# Plot distribution of signals
plt.figure(figsize=(10,6))
sns.countplot(data=df, x="symbol", hue="signal")
plt.title("Distribution of Signals (LONG vs FLAT)")
plt.xlabel("Symbol")
plt.ylabel("Count")
plt.tight_layout()
plt.savefig("../images/09_paper_signal_distribution.png")
plt.close()

# Plot average predicted return per symbol
avg_pred = df.groupby("symbol")["predicted_return"].mean()
avg_pred.plot(kind="bar", figsize=(10,6))
plt.title("Average Predicted Return per Symbol")
